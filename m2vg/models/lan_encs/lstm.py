...
from m2vg.models import LAN_ENCODERS
...
class LSTM(nn.Module):
    def __init__(
        ...
        word_emb,
        lstm_cfg=dict(
            type="gru", num_layers=1, dropout=0.0, hidden_size=512, bias=True, bidirectional=True, batch_first=True
        ),
        ...
        if self.output_type == "query":
            self.parser = nn.ModuleList([PhraseAttention(input_dim=lstm_cfg["hidden_size"] * 2) for _ in range(4)])
    ...
    def forward(self, ref_expr_inds):
        ...
        if self.output_type == "mean":
            y = torch.cat(
                list(map(lambda feat, mask: torch.mean(feat[mask, :], dim=0, keepdim=True), y_word, ~y_mask))
            ).unsqueeze(1)
        elif self.output_type == "max":
            y = torch.cat(
                list(map(lambda feat, mask: torch.max(feat[mask, :], dim=0, keepdim=True)[0], y_word, ~y_mask))
            ).unsqueeze(1)
            ...
            res = {"text_feat": y_word, "text_mask": y_mask}
            return res
