...
from m2vg.models import VIS_ENCODERS
from m2vg.utils import get_root_logger, is_main
from m2vg.models.utils import freeze_params, parse_yolo_weights
...
class CSPDarknet(BaseModule):
    ...
    def __init__(
        ...
        self.do_train = False if self.frozen_stages >= 0 else True
        self.load_pretrain()
        self.train()
    ...
    def load_pretrain(self):
        if "prefix" in self.init_cfg:
            prefix = self.init_cfg["prefix"]
            ckpt = self.init_cfg["checkpoint"]
            checkpoint = torch.load(ckpt)["state_dict"]
            filtered_checkpoint = {key.replace(prefix, ""): value for key, value in checkpoint.items() if prefix in key}
            missing_key, unexpected_key = self.load_state_dict(filtered_checkpoint, strict=False)
            if is_main():
                self.logger.info("loadding visual encoder checkpoint from {}".format(ckpt))
                self.logger.info("visual_encoder_missing_key:{}".format(missing_key))
                self.logger.info("visual_encoder_missing_key:{}".format(unexpected_key))
