...
class BEIT3(BEiT3Wrapper):
    def __init__(
        ...
        if freeze_layer >= 0:
            self.frozen_stages = (
                freeze_layer if freeze_layer <= len(self.beit3.encoder.layers) else len(self.beit3.encoder.layers)
            )
            self._freeze_stages()
            ...
            if pos_embed_key in checkpoint_model:
                ...
                if orig_size != new_size:
                    ...
                    if torchscale_model:
                        ...
                        pos_tokens = pos_embed_checkpoint[:, num_extra_tokens:]
                    pos_tokens = (
                        pos_tokens.reshape(-1, orig_size, orig_size, embedding_size).permute(0, 3, 1, 2).float()
                    )
