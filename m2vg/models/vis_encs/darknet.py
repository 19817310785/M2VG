...
def darknet_conv(
    ...
    for i, (in_ch, out_ch, kernel_size, stride) in enumerate(zip(in_chs, out_chs, kernel_sizes, strides)):
        convs.append(
            ConvModule(
                in_ch, out_ch, kernel_size, stride=stride, padding=kernel_size // 2, norm_cfg=norm_cfg, act_cfg=act_cfg
            )
        )
...
class DarknetBlock(nn.Module):
    def __init__(self, ch, num_block=1, shortcut=True):
        super(DarknetBlock, self).__init__()
        self.shortcut = shortcut
        self.module_list = nn.ModuleList(
            [nn.ModuleList([*darknet_conv((ch, ch // 2), (ch // 2, ch), (1, 3), (1, 1))]) for _ in range(num_block)]
        )
