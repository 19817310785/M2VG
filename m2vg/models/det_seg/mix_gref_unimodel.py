...
import torch
import numpy
from m2vg.core.structure.boxes import Boxes
from m2vg.core.structure.instances import Instances
from m2vg.core.structure.postprocessing import detector_postprocess
from m2vg.layers.box_ops import box_cxcywh_to_xyxy
from m2vg.models import MODELS
from mmdet.core import BitmapMasks
import pycocotools.mask as maskUtils

from m2vg.models.postprocess.nms import nms
from .one_stage import OneStageModel
import numpy as np
from PIL import Image, ImageDraw
from m2vg.utils import is_main
import os
import math
...
#from m2vg.models.losses.contrastive_loss import WordRegionContrastiveLoss
# 放到 mix_gref_unimodel.py 的 import 语句下方
...
class AdaptiveWordAwareResidualModulation(nn.Module):
    """Visual-context token residual modulation used before query generation."""
    ...
    def __init__(
        ...
        self.visual_value = nn.Linear(visual_dim, text_dim)
    ...
    def forward(self, text_feat, text_padding_mask, img_feat):
        if not self.enabled or text_feat is None:
            return text_feat

        valid_mask = self._valid_token_mask(text_feat, text_padding_mask)

        if self.use_visual_context and img_feat is not None:
            visual_tokens = F.adaptive_avg_pool2d(img_feat, (self.pool_size, self.pool_size))
            visual_tokens = visual_tokens.flatten(2).transpose(1, 2)
            query = self.text_query(text_feat)
            key = self.visual_key(visual_tokens)
            attn = torch.matmul(query, key.transpose(1, 2)) / math.sqrt(query.shape[-1])
            attn = torch.softmax(attn, dim=-1)
            visual_context = torch.matmul(attn, self.visual_value(visual_tokens))
            update_feat = visual_context
        else:
            visual_context = torch.zeros_like(text_feat)
            update_feat = text_feat

        if self.use_sentence_context:
            sentence_feat = (text_feat * valid_mask.unsqueeze(-1)).sum(1)
            sentence_feat = sentence_feat / valid_mask.sum(1, keepdim=True).clamp_min(1.0)
            sentence_feat = sentence_feat.unsqueeze(1).expand_as(text_feat)
        else:
            sentence_feat = torch.zeros_like(text_feat)

        gate_input = torch.cat(
            [text_feat, visual_context, sentence_feat, text_feat * visual_context],
            dim=-1,
        )
        coeff = torch.sigmoid(self.gate(gate_input)) * 2.0 - 1.0
        coeff = coeff * valid_mask.unsqueeze(-1)
        if self.use_residual:
            return text_feat + self.residual_scale * coeff * update_feat
        return text_feat * (1.0 + coeff)

    def _valid_token_mask(self, text_feat, text_padding_mask):
        if text_padding_mask is None:
            return text_feat.new_ones(text_feat.shape[:2])
        valid_mask = (text_padding_mask == 0).float()
        alt_valid_mask = (text_padding_mask > 0).float()
        has_valid = valid_mask.sum(1, keepdim=True) > 0
        return torch.where(has_valid, valid_mask, alt_valid_mask).to(text_feat.device)



@MODELS.register_module()
class MIXGrefUniModel(OneStageModel):
    def __init__(
        self,
        word_emb,
        num_token,
        ...
        super(MIXGrefUniModel, self).__init__(word_emb, num_token, vis_enc, lan_enc, head, fusion)
        ...
        self.exist_threshold = post_params.get("exist_threshold", 0.5)
        self.use_exist_inference = post_params.get("use_exist_inference", True)
        awrm_cfg = awrm_cfg or {}
        self.awrm = AdaptiveWordAwareResidualModulation(
            text_dim=awrm_cfg.get("text_dim", 768),
            visual_dim=awrm_cfg.get("visual_dim", 768),
            hidden_dim=awrm_cfg.get("hidden_dim", 256),
            pool_size=awrm_cfg.get("pool_size", 4),
            enabled=awrm_cfg.get("enabled", True),
            use_visual_context=awrm_cfg.get("use_visual_context", True),
            use_sentence_context=awrm_cfg.get("use_sentence_context", True),
            use_residual=awrm_cfg.get("use_residual", True),
        )
    ...
    def forward_train(
        ...
        gt_exist=None,  # 👉 【新增这一行】：接收数据端传来的 existence 标签
        rescale=False,
        epoch=None,
    ):
        """Args:
        ...
        text_feat = self.awrm(text_feat, text_attention_mask, img_feat)
    ...
    def get_predictions(self, pred, img_metas, rescale=False, with_bbox=False, with_mask=False):
        ...
        pred_exist = pred.get("pred_exist", None) if self.use_exist_inference else None
        pred_exist_prob = pred_exist.sigmoid() if pred_exist is not None else None
        if bboxes is not None and with_bbox:
            ...
            for ind, (results_per_image, img_meta) in enumerate(zip(results, img_metas)):
                ...
                cur_exist_prob = pred_exist_prob[ind] if pred_exist_prob is not None else None
                if cur_exist_prob is not None:
                    score = score * cur_exist_prob
                    if cur_exist_prob <= self.exist_threshold:
                        score = score * 0.0
                if rescale:
        ...
        if mask_seg is not None and with_mask:
            ...
            for ind, (mask, img_meta) in enumerate(zip(mask_binary, img_metas)):
                cur_exist_prob = pred_exist_prob[ind] if pred_exist_prob is not None else None
                if cur_exist_prob is not None and cur_exist_prob <= self.exist_threshold:
                ...
                if cur_exist_prob is not None:
                    pred_nt = torch.stack((cur_exist_prob, 1.0 - cur_exist_prob))
                gt_nt = img_meta["empty"]
                pred_masks.append(
                    {
                        "pred_masks": pred_rle,
                        "pred_nt": pred_nt,
                        "pred_exist": cur_exist_prob,
    ...
    def get_predictions_parts(self, pred, img_metas, rescale=False, with_bbox=False, with_mask=False):
        ...
        if mask_seg is not None and with_mask:
            ...
            for ind, (mask, img_meta, global_mask) in enumerate(zip(mask_binary, img_metas, global_mask_binary)):
                ...
                if cur_exist_prob is not None and cur_exist_prob <= self.exist_threshold:
                    mask_ = torch.zeros_like(global_mask[0]).int()
                elif self.outmask_type == "global":
