...
import numpy as np
from m2vg.models import MODELS, build_head
from .unet_head import *
from m2vg.models.losses.boxloss import BoxLoss
...
class QueryAugment(nn.Module):
    ...
    def forward(self, box_feat, img_feat, lan_feat, lan_mask):
        ...
        text_pos_embed = (
            self.position_embedding_1d(lan_feat).unsqueeze(0).repeat(lan_feat.shape[0], 1, 1).permute(1, 0, 2).cuda()
        )
...
class BoxSegAttention(nn.Module):
    ...
    def forward(self, box_feat, seg_feat, img_feat, lan_feat=None, lan_mask=None):
        ...
        box_seg_query = self.segboxSA(
            query=box_seg_query, key=None, value=None, query_pos=box_seg_pos_embed, query_key_padding_mask=None
        )
...
class UnifiedInteractionModule(nn.Module):
    def __init__(
        self, input_channels=256, box_weights=[0.1, 1.0], weighted_compose="none", enable_box_coorinate_embed=False
    ):
    ...
    def forward(self, pred_box, pred_mask, img_feat, lan_feat=None, lan_mask=None):
        ...
        extra = {"unified_img_feat": unified_heatmap, "img_feat": img_feat_norm}
