...
class PredictHead(nn.Module):
    ...
    def forward(self, x, lan_feature=None, lan_mask=None):
        if self.name == "CRIS":
            lan_feature = torch.cat(
                list(map(lambda feat, mask: torch.max(feat[mask, :], dim=0, keepdim=True)[0], lan_feature, ~lan_mask))
            )
...
class UnetHead(nn.Module):
    def __init__(
        self, input_channels, patch_size, fpn_output_channels=[96, 192, 384, 768], predict_head="Conv", fpn="None"
    ):
...
class SimpleFPN(BaseModule):
    ...
    def __init__(
        ...
        for i in range(self.num_ins):
            l_conv = ConvModule(
                in_channels[i], out_channels[i], 1, conv_cfg=conv_cfg, norm_cfg=norm_cfg, act_cfg=act_cfg, inplace=False
            )
            fpn_conv = ConvModule(
                out_channels[i],
                out_channels[i],
...
class UnetDecoder(nn.Module):
    def __init__(self, c4_dims, factor=1):
