...
import torch
from m2vg.core.criterion.criterion import SetCriterion
from m2vg.core.matcher import HungarianMatcher
from m2vg.layers.box_ops import box_cxcywh_to_xyxy, box_xyxy_to_cxcywh
from m2vg.layers.mlp import MLP
from m2vg.layers.position_embedding import (
    PositionEmbeddingSine,
    PositionEmbeddingSine1D,
)
from m2vg.models import HEADS
import pycocotools.mask as maskUtils
import numpy as np

from m2vg.models.heads.tgqs_kd_detr_head.transformer import (
    DetrTransformer,
    DetrTransformerDecoder,
    DetrTransformerEncoder,
)
from m2vg.models.heads.transformers.deformable_transformer import (
...
from m2vg.models.losses.segloss import refer_ce_loss, part_seg_loss, seg_loss
from m2vg.utils.visual_utils import (
...
class UniGRefDeformableHead(nn.Module):
    def __init__(
        ...
        criterion_cfg = detr_loss.get("criterion", {})
        self.exist_loss_weight = criterion_cfg.get("loss_exist", 0.0)
        self.exist_topk = criterion_cfg.get("exist_topk", min(3, num_queries))
        self.exist_alpha = criterion_cfg.get("exist_alpha", 0.25)
        self.exist_gamma = criterion_cfg.get("exist_gamma", 2.0)
        self.exist_embed = MLP(
            input_dim=hidden_channels,
            hidden_dim=hidden_channels,
            output_dim=1,
            num_layers=3,
    ...
    def predict_exist(self, query_feat, output_class):
        foreground_score = F.softmax(output_class, dim=-1)[:, :, :-1].max(-1).values
        topk = min(self.exist_topk, query_feat.shape[1])
        topk_score, topk_index = torch.topk(foreground_score, topk, dim=1)
        gather_index = topk_index.unsqueeze(-1).expand(-1, -1, query_feat.shape[-1])
        topk_feat = torch.gather(query_feat, 1, gather_index)
        weights = torch.softmax(topk_score.detach(), dim=1).unsqueeze(-1)
        expression_feat = (topk_feat * weights).sum(1)
        return self.exist_embed(expression_feat).squeeze(-1)

    def get_exist_targets(self, targets, is_empty, device):
        target_exist = targets.get("exist", None)
        if target_exist is None:
            return (1 - is_empty.float()).to(device)
        if isinstance(target_exist, torch.Tensor):
            return target_exist.to(device).float().view(-1)
        if isinstance(target_exist, (list, tuple)):
            tensors = []
            for item in target_exist:
                if isinstance(item, torch.Tensor):
                    tensors.append(item.to(device).float().view(-1)[0])
                else:
                    tensors.append(torch.as_tensor(item, device=device).float().view(-1)[0])
            return torch.stack(tensors).view(-1)
        return torch.as_tensor(target_exist, device=device).float().view(-1)

    def exist_focal_loss(self, pred_exist, target_exist):
        ce_loss = F.binary_cross_entropy_with_logits(pred_exist, target_exist, reduction="none")
        prob = pred_exist.sigmoid()
        p_t = prob * target_exist + (1 - prob) * (1 - target_exist)
        loss = ce_loss * ((1 - p_t) ** self.exist_gamma)
        if self.exist_alpha >= 0:
            alpha_t = self.exist_alpha * target_exist + (1 - self.exist_alpha) * (1 - target_exist)
            loss = alpha_t * loss
        return loss.mean()
    ...
    def forward_train(self, x, targets, cls_feat=None, lan_feat=None, lan_mask=None, img=None):
        ...
        pred_exist = self.predict_exist(inter_states[-1], outputs_class[-1])
        ...
        target_exist = self.get_exist_targets(targets, is_empty, device)
        # ! detr loss
        ...
        loss_exist = self.exist_focal_loss(pred_exist, target_exist) * self.exist_loss_weight
        # ! part seg loss
