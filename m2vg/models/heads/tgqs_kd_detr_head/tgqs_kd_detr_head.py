...
from m2vg.layers.box_ops import box_cxcywh_to_xyxy, box_xyxy_to_cxcywh, box_iou
from m2vg.layers.mlp import MLP
from m2vg.core.criterion import SetCriterion
from m2vg.layers.position_embedding import PositionEmbeddingSine, PositionEmbeddingSine1D
from m2vg.core.matcher import HungarianMatcher
from m2vg.core.structure import Boxes, Instances
from m2vg.models.utils import freeze_params
from .transformer import DetrTransformer, DetrTransformerEncoder, DetrTransformerDecoder
from m2vg.models import HEADS
from m2vg.core.criterion.distill_criterion import DistillCriterion
...
class TextGuidedQuerySelectKDDETRHead(nn.Module):
    def __init__(
        ...
        assert all(
            x in ["decoder", "token", "distill", "merge", "aux_distill", "balanced_distill"]
            for x in branch_loss_weight.keys()
        )
        ...
        if num_token_mlp_layers > 0:
            self.mlp = MLP(
                ...
                num_layers=num_token_mlp_layers,
                return_intermediate=True,
            )
        else:
        ...
        for target_bbox, img_meta in zip(targets, img_metas):
            ...
            if len(target_bbox.shape) == 1:
                ...
                gt_classes = torch.tensor(
                    [1 if t["category_id"] == -1 else 0 for t in img_meta["target"]], device=target_bbox.device
                ).long()
    ...
    def prepare_soft_targets(
        self, targets, decoder_branch_output, img_metas, predict_threahold=0.0, prepare_target_mode="iou_weighted"
    ):
        new_targets_pred = []
        new_targets_gt = []
        ...
        if prepare_target_mode == "score_weighted":
            ...
            for indice, predict_bbox, predict_score, target_gt, img_meta in zip(
                indices, decoder_pred_boxes, decoder_scores, new_targets_gt_out, img_metas
            ):
    ...
    def calc_distill_loss(
        ...
        if distill_type == "soft":
            ...
            loss_dict = self.criterion_softdistill.forward_train_distill(
                all_cls_scores, all_bbox_preds, img_metas, teacher_bboxes, teacher_labels
            )
    ...
    def forward_general(self, x_mm, img_metas, cls_feat=None, text_feat=None, text_mask=None):
        ...
        if self.text_guided_query_generation:
            ...
            text_feat_filter = (
                torch.cat(
                    list(
                        map(lambda feat, mask: torch.max(feat[mask, :], dim=0, keepdim=True)[0], text_feat, ~text_mask)
                    )
                )
                .unsqueeze(1)
            ...
            text_pos_embed = (
                self.position_embedding_1d(text_feat)
                .unsqueeze(0)
                .repeat(text_feat.shape[0], 1, 1)
                .permute(1, 0, 2)
                .cuda()
            )
    ...
    def forward_train(self, x_mm, img_metas, cls_feat=None, text_feat=None, gt_bbox=None, text_mask=None):
        ...
        targets_gt, targets_predict = self.prepare_soft_targets(
            ...
            decoder_branch_output,
            img_metas,
            predict_threahold=self.as_target_query_thr,
        ...
        if "decoder" in self.branch_loss_weight:
            loss_dict_decoder_branch = self.calc_loss(
                outputs_class_decoder_branch, outputs_coord_decoder_branch, targets_gt
            )
        ...
        if "balanced_distill" in self.branch_loss_weight:
            ...
            loss_dict_token_branch_gt = self.calc_loss(
                outputs_class_token_branch_, outputs_coord_token_branch_, targets_gt
            )
            loss_token_branch = sum(loss_dict_token_branch_gt.values())
            loss_token_branch = (
                self.branch_loss_weight["balanced_distill"]["token"] * loss_token_branch * (1 - weights_distill)
            )
            loss_dict["loss_tgt"] = loss_token_branch

            loss_dict_token_branch_predict = self.calc_loss(
                outputs_class_token_branch_, outputs_coord_token_branch_, targets_predict
            )
        ...
        if "merge" in self.branch_loss_weight:
            ...
            loss_dict_merge_branch_gt = self.calc_loss(
                outputs_class_token_branch, outputs_coord_token_branch, targets_merge
            )
        ...
        if "aux_distill" in self.branch_loss_weight:
            assert (
                token_features.shape == decoder_features.shape
            ), "the number of the mlp layers and ecoder layers must be the same!!"
            decoder_output = []
            token_output = []
            for decoder_class, decoder_coord, token_class, token_coord in zip(
                outputs_class_decoder_branch[:-1],
                outputs_coord_decoder_branch[:-1],
                outputs_class_token_branch[:-1],
                outputs_coord_token_branch[:-1],
            ):
            ...
            for decoder_o, token_o in zip(decoder_output, token_output):
                targets_gt_aux, targets_predict_aux = self.prepare_soft_targets(
                    gt_bbox,
                    decoder_o,
                    img_metas,
                    predict_threahold=self.as_target_query_thr,
    ...
    def forward_test(
        self, x_mm, img_metas, text_feat=None, cls_feat=None, with_bbox=False, with_mask=False, text_mask=None
    ):
