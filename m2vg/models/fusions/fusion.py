...
from m2vg.models import FUSIONS
from m2vg.models.vis_encs.darknet import darknet_conv


@FUSIONS.register_module()
class SimpleFusion(nn.Module):
    def __init__(self, vis_chs=(256, 512, 1024), direction="bottom_up"):
        ...
        if direction == "bottom_up":
            ...
            ch = sum(vis_chs[:2])
            self.down_mid2top = nn.Sequential(nn.AvgPool2d(2, 2), *darknet_conv((ch,), (ch,), (3,), (1,)))
            self.down_bot2mid = nn.Sequential(
                nn.AvgPool2d(2, 2), *darknet_conv((vis_chs[0],), (vis_chs[0],), (3,), (1,))
            )
            ...
            self.up_top2mid = nn.Sequential(
                nn.UpsamplingBilinear2d(scale_factor=2), *darknet_conv((vis_chs[-1],), (vis_chs[-1],), (3,), (1,))
            )
            self.down_bot2mid = nn.Sequential(
                nn.AvgPool2d(2, 2), *darknet_conv((vis_chs[0],), (vis_chs[0],), (3,), (1,))
            )
...
class SimpleFusionv3(nn.Module):
    ...
    def forward(self, x, y):
        ...
        x_multi_modal = self.activate(self.linear_vision(x_vis_enc * self.scale)) * self.activate(
            self.linear_text(y_2d)
        )
