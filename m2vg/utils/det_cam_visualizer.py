...
from m2vg.datasets.pipelines import Compose
from m2vg.models import build_model
...
def reshape_transform(feats, max_shape=(20, 20), is_need_grad=False):
    ...
    feats = feats[:, 1:-20].transpose(-1, -2).reshape(1, -1, 16, 16)
    ...
    def __call__(self, *args, **kwargs):
        if self.return_loss:
            ...
            with torch.no_grad():
                ...
                results = self.detector(return_loss=False, rescale=True, **self.input_data)[0]
                bboxes = results["pred_bboxes"]
                labels = results["predict_classes"]

                return [
                    {"bboxes": bboxes.cpu().detach().numpy(), "labels": labels.cpu().detach().numpy(), "segms": None}
                ]
...
class DetCAMVisualizer:
    ...
    def __init__(
        self, method_class, model, target_layers, reshape_transform=None, is_need_grad=False, extra_params=None
    ):
    ...
    def switch_activations_and_grads(self, model):
        ...
        if self.is_need_grad is True:
            self.cam.activations_and_grads = ActivationsAndGradientsBeiT(
    ...
    def show_cam(self, image, boxes, labels, grayscale_cam, with_norm_in_bboxes=False):
        ...
        image_with_bounding_boxes = self._draw_boxes(boxes, labels, cam_image_renormalized)
