...
def pred_no_target(pred_bbox, eval_sample, threshold):
    if eval_sample.get("pred_exist", None) is not None:
        pred_exist = eval_sample["pred_exist"]
        exist_threshold = eval_sample.get("exist_threshold", 0.5)
        if torch.is_tensor(pred_exist):
            return (pred_exist <= exist_threshold).to("cpu")
        return torch.tensor(pred_exist <= exist_threshold)
    return ((pred_bbox["scores"] > threshold).sum() == 0).to("cpu")
...
def gres_evaluate(pred_bboxes, pred_masks, gt_masks, threshold=0.7):
    ...
    for ind, (pred_bbox, eval_sample) in enumerate(zip(pred_bboxes, pred_masks)):
        ...
        # ref_result["pred_nt"] = (((pred_bbox["scores"] * eval_sample["pred_nt"][0])>threshold).sum()==0).to("cpu")
        ref_result["pred_nt"] = pred_no_target(pred_bbox, eval_sample, threshold)
        # ref_result["pred_nt"] = False
        # ref_result["pred_nt"] = eval_sample["pred_nt"].argmax(dim=0).bool().to("cpu")
...
def grec_evaluate_f1_nacc(
    predictions, gt_bboxes, img_metas, thresh_score=0.7, thresh_iou=0.5, thresh_F1=1.0, with_nms=False
):
    correct_image = 0
    num_image = 0
    nt = {
    ...
    for prediction, gt_bbox, target in zip(predictions, gt_bboxes, img_metas):
        ...
        # if with_nms:
        #     filtered_indices = nms(filtered_boxes, torch.as_tensor(sorted_scores_array[idx]), 0.8)
