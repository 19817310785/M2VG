...
class HungarianMatcher(nn.Module):
    ...
    def __init__(
        ...
        self.alpha = alpha
        self.gamma = gamma
        assert cost_class != 0 or cost_bbox != 0 or cost_giou != 0 or cost_point != 0, "all costs cant be 0"
    ...
    def forward(self, outputs, targets):
        ...
        # Compute the point cost betwen pred_point and box center
        if outputs["pred_selected_points"] is not None:
            pred_selected_points = outputs["pred_selected_points"].flatten(0, 1)
            cost_point = torch.cdist(pred_selected_points, tgt_bbox[:, :2], p=1)
