...
from m2vg.layers.box_ops import box_cxcywh_to_xyxy, generalized_box_iou
from m2vg.utils.dist import get_world_size, is_dist_avail_and_initialized
from typing import List
import torch
import torch.nn as nn
import torch.nn.functional as F

from torchvision.ops import sigmoid_focal_loss as tv_focal_loss  # <-- 【新增】引入 torchvision 的 focal loss
...
class SetCriterion(nn.Module):
    ...
    def __init__(
        ...
        if self.loss_class_type in ["ce_loss", "weighted_ce_loss"]:
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes):
        ...
        if self.loss_class_type == "weighted_ce_loss":
            target_weights_o = torch.cat(
                [
                    (
                        t["weight"][J]
                        if not len(t["weight"][J]) == 0
                        else torch.tensor([0], device=targets[0]["weight"].device)
                    )
                    for t, (_, J) in zip(targets, indices)
                ]
            ).squeeze()
    ...
    def loss_exist(self, outputs, targets, indices, num_boxes, **kwargs):
        """
        计算目标存在性 (Existence) 的 Focal Loss
        """
        assert "pred_exist" in outputs, "outputs must contain 'pred_exist' for non-target discrimination"

        # 获取模型预测的存在性 logits，通常形状为 (batch_size, 1) 或 (batch_size,)
        pred_exist = outputs["pred_exist"]

        # 从 targets 中提取我们通过 CollectData 传过来的 gt_exist 标签
        # 统一维度形状以匹配 pred_exist
        target_exist = torch.stack([t["exist"] for t in targets]).float().view_as(pred_exist)

        # 使用 torchvision 的 focal loss 处理难负样本
        loss_exist = tv_focal_loss(
            pred_exist,
            target_exist,
            alpha=self.alpha,
            gamma=self.gamma,
            reduction="mean"  # 存在性是图像级别的全局判断，直接取均值即可
        )
    ...
    def get_loss(self, loss, outputs, targets, indices, num_boxes, **kwargs):
        loss_map = {
            ...
            "exist": self.loss_exist,  # <-- 【修改】新增存在性损失的分发路由
        }
