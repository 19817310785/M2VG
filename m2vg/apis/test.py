...
from m2vg.evaluations.grefcoco import grec_evaluate_f1_nacc, gres_evaluate
from m2vg.evaluations.refzom import refzom_evaluate
from m2vg.evaluations.rrefcoco import rrefcoco_evaluate
...
import numpy as np
import copy
from m2vg.utils.dist import all_gather, synchronize
...
def accuracy(pred_bboxes, gt_bboxes, pred_masks, gt_masks, is_crowd=None, device="cuda:0"):
    ...
    for ind, (ppred_bbox, gt_bbox, eval_sample) in enumerate(zip(pred_bboxes, gt_bboxes, pred_masks)):
        if "filtered_scores" in ppred_bbox:
            ...
            sorted_scores_boxes = sorted(zip(ppred_bbox["scores"].tolist(), ppred_bbox["boxes"].tolist()), reverse=True)
...
def evaluate_model(prefex, cfg, model, loader):
    ...
    pred_bboxes, gt_bboxes, pred_masks, gt_masks, img_meta_total, cover_acc = [], [], [], [], [], []
    start = time.time()
    ...
    with torch.no_grad():
        for batch, inputs in enumerate(loader):
            ...
            if cfg.distributed:
                ...
                gt_bboxes.extend(list(itertools.chain(*all_gather(gt_bbox))))
