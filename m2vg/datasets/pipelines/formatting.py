...
class DefaultFormatBundle:
    ...
    def __call__(self, results):
        ...
        if "gt_exist" in results:
            results["gt_exist"] = DataContainer(to_tensor(results["gt_exist"]), stack=True, pad_dims=None)

        if results["with_bbox"]:
            results["gt_bbox"] = DataContainer(to_tensor(numpy.array(results["gt_bbox"])))
        ...
        if results["with_mask"]:
            ...
            if "gt_mask_parts_rle" in results:
                results["gt_mask_parts_rle"] = DataContainer(results["gt_mask_parts_rle"], cpu_only=True, stack=False, pad_dims=None)
