...
class BaseDataset(Dataset):
    def __init__(self, imgsfile, annsfile, pipeline, which_set="train", img_source=["coco"], word_emb_cfg=None):
        ...
        self.expression_pool = self._build_expression_pool(self.anns_all)
    ...
    def __getitem__(self, index):
        results = {
            ...
            "expression_pool": self.expression_pool,
        }
    ...
    def _extract_raw_expressions(self, ann):
        expressions = ann.get("expressions", "")
        if isinstance(expressions, dict):
            expressions = expressions.get("raw", "")
    ...
    def _extract_category_ids(self, ann):
        category_ids = set()
        ...
        if category_id is not None:
            category_ids.add(category_id)
        return tuple(sorted(category_ids))

    def _build_expression_pool(self, anns):
        pool = []
        for ann in anns:
            for ann_dict in self._iter_ann_dicts(ann):
                category_ids = self._extract_category_ids(ann_dict)
                for text in self._extract_raw_expressions(ann_dict):
                    text = text.strip()
                    if text:
                        pool.append({"text": text, "category_ids": category_ids})
        return pool
