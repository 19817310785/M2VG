...
from m2vg.apis import evaluate_model, set_random_seed
...
def main_worker(cfg):
    ...
    if cfg.dataset == "RRefCOCO":
        ...
        if not getattr(cfg, "only_val", False) and hasattr(cfg.data, "testA") and hasattr(cfg.data, "testB"):
            datasets_cfgs.append(cfg.data.testA)
            datasets_cfgs.append(cfg.data.testB)
            prefix.extend(["testA", "testB"])
        elif not getattr(cfg, "only_val", False) and hasattr(cfg.data, "test"):
    ...
    if is_main():
        ...
        target_excel_path = os.path.join(
            ...
            "output_thr{}_{}_{}_{}.xlsx".format(
                ...
                cfg.model.post_params["outmask_type"],
            ),
        )
