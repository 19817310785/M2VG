...
from m2vg.core import build_optimizer, build_scheduler
from m2vg.datasets import build_dataset, build_dataloader
from m2vg.models import build_model, ExponentialMovingAverage
from m2vg.apis import set_random_seed, train_model, evaluate_model
from m2vg.utils import (
    get_root_logger,
    load_checkpoint,
    save_checkpoint,
...
def main_worker(cfg):
    ...
    if is_main():
        ...
        wandb.init(project="m2vg-offline", tags=[cfg.tag_name], name=cfg.wandb_name, mode="offline")
    ...
    train_params = [
        ...
        {
            "params": [
                p for n, p in model.named_parameters() if "lan_enc" not in n and "vis_enc" not in n and p.requires_grad
            ],
...
def main():
    ...
    cfg.wandb_name = "-".join(cfg.work_dir.split("/")[1:-1])
    cfg.tag_name = "-".join(cfg.work_dir.split("/")[1:-2])
